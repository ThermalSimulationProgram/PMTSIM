 function [optT, solution] = findTheOptSolutionByGradient(TM, config, dynamicData)
    ...
    if sum(toffs) + step <= limitSumToffs
        ...
        for i = 1 : acn % the increased one
            ...
            for j = 1 : acn % the decreased one
                ...
                nextIdx = oldIndex;
                nextIdx(i) = oldIndex(i) + 1;
                nextIdx(j) = oldIndex(j) - 1;
                [toffs, ~] = getPTMs(candids, nextIdx);
        
                [tact, tslp]= prepareTacts(toffs, K, config);
    [nextTemp, Timp, ~] = CalculatePeakTemperatureV3(isFast, TM, tslp, tact, Timp);
